...
const poll = {
    ...
    registerNewAnswer() {
        ...
        if (answer <= 3 && answer >= 0) {
            this.answers[answer]++;
        } else if (answer > 3 || answer < 0 || isNaN(answer)) {
            alert('Please insert a correct number!');
        }

        //Task 4
        this.displayResults(); //Default
...
const testAnswer = poll.displayResults;
testAnswer.call({answers: data1}, 'array');
testAnswer.call({answers: data1}, 'string');

testAnswer.call({answers: data2}, 'array');
testAnswer.call({answers: data2}, 'string');